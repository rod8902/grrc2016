...
//---------------------------------------------------------------------------
// ExampleIIRCall() shows how to use the functions in this file.
// These 3 define's and include's are only needed for ExampleIIRCall.
...
void ExampleIIRCall(void)
{
 ...
 TIIRPassTypes PassType = iirLPF;       // iirLPF, iirHPF, iirBPF, iirNOTCH, iirALLPASS
...
TIIRCoeff CalcIIRFilterCoeff(TIIRFilterTypes ProtoType, double Beta, TIIRPassTypes PassType, int NumPoles, double OmegaC, double BW)
{
 ...
 // All Pass filters have s plane numerators equal the denominators except to negate the 1st order coefficient.
 // H(s) = ( As^2 + -Bs + C ) / ( As^2 + Bs + C )
 // In the s plane, the zeroes are reflected about the imag axis from the poles.
 // In the z plane, the zeroes are reflected about the unit circle from the poles.
 // Use the Gauss for the best results (most linear phase).
 if(PassType == iirALLPASS)
  {
   for(j=0; j<SPlaneCoeff.NumSections; j++)
    {
     SPlaneCoeff.D[j] = SPlaneCoeff.A[j];
     SPlaneCoeff.E[j] = -SPlaneCoeff.B[j];
     SPlaneCoeff.F[j] = SPlaneCoeff.C[j];
    }
  }
 ...
 for(j=0; j<SPlaneCoeff.NumSections; j++)
  ...
  if(PassType == iirLPF || PassType == iirALLPASS)
   {
